...
public class LoginServlet extends HttpServlet {
	...
	public void init() throws ServletException {
		try {
			...
			// ServletException(String errmessge,throwable rootcause)
		...
		try (PrintWriter pw = response.getWriter()) {
			...
			if (user == null) // login failed
				pw.print("<h4> Invalid Email or Password , Please <a href='login.html'>Retry</a></h4>");
					...
					// create a cookies to hold user details
					// javax.servlet.http.cokie(string nm,strinhgvalue)
				Cookie c1 = new Cookie("user_dtls", user.toString());
				// add the cookie to resp header
				// method of httpservletresp:public void addcokkie(cookie c)
				...
				if (user.isVotingStatus()) {// alrdy voted
					...
					// WC clears (i.e empties?discards)the resp buffer
					// sends temp redirect resp : SC 302 |
					// location=candidate_list,Set-Cookie:user_dtls:user details.toString()
					// | body : EMPTY
					// web browser : checks privacy setting--cookies are blocked=>session tracking fails
					//chcks age(expiry):-1=>stores the cookies in cache
